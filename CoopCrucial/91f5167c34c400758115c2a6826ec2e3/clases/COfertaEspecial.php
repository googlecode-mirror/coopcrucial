...
class COfertaEspecial {
    ...
    public function obtenerListadoOfertasEspeciales() {
        ...
        $sentencia = "SELECT * FROM `oferta_especial` ORDER BY idOfertaEspecial ASC";
    ...
    //Especifico consecutivo Oferta Especial
    public function obtenerConsecutivoOfertaEspecial() {
        $this->conectar();

        mysql_select_db($this->database_conn, $this->conn);
        $sentencia = "SELECT * FROM `oferta_especial` ORDER BY idOfertaEspecial DESC";
        $resultado = mysql_query($sentencia, $this->conn) or die(mysql_error());
        $fila = mysql_fetch_assoc($resultado);
        return $fila[idOfertaEspecial] + 1;
    }
