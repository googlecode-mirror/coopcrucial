...
// NEW ELEMENTS & CACHE
				...
				var $magnifier = $('<li class="etalage_magnifier"><div><img /></div></li>').appendTo($container),
					$magnifier_img_area = $magnifier.find('div'),
					$magnifier_img = $magnifier_img_area.find('img');
				...
				// Add zoom area
				var $zoom;
				if(o.zoom_element!=='auto' && o.zoom_element && $(o.zoom_element).length){
					$zoom = $(o.zoom_element)
				...
				if(o.show_descriptions){
					$description = $('<div class="etalage_description"></div>').prependTo($zoom);
				}
				...
				if(images > 1 || !o.smallthumb_hide_single){
					...
					$smallthumbs_ul = $smallthumbs.find('ul');
					$.each($thumb_images, function(){
						var $t = $(this);
						src = $t.data('src');
					...
					$smallthumb = $smallthumbs.find('li').css({ opacity:o.smallthumb_inactive_opacity });
					if(o.small_thumbs < 3){
						o.small_thumbs = 3;
					}
					...
					$smallthumb_images = $smallthumb.find('img');
...
// PREPARE
				...
				var zoom_border = parseInt($zoom.css('borderTopWidth'), 10),
					zoom_margin = parseInt(o.zoom_area_distance),
				...
				if(o.show_descriptions){
					...
					description_width = o.zoom_area_width - description_border - description_margin - description_padding;
				}
				...
				var magnifier_width = Math.round(o.zoom_area_width*(o.thumb_image_width / o.source_image_width)),
					magnifier_height = Math.round(o.zoom_area_height*(o.thumb_image_height / o.source_image_height)),
...
// RESIZE AND REPOSITION ELEMENTS
				...
				if(o.align==='right' && o.zoom_element==='auto'){
					$zoom.css({ left:-(o.zoom_area_width + (zoom_border*2) + (zoom_padding*2) + zoom_margin) });
				}else if(o.zoom_element==='auto'){
					$zoom.css({ left:container_outerwidth+zoom_margin });
				}
				$zoom.css({ margin:0 }).css({ opacity:0 }).hide();
				if(o.show_descriptions){
					css = { width:description_width, bottom:zoom_padding, left:zoom_padding };
					if(!ie){
						css.opacity = o.description_opacity;
					}
...
// FUNCTIONS
				...
				var st_click = function($next_active, moved){
					...
					var $next_thumb = $thumbs.filter('.thumb_'+$next_active.data('thumb_id')).addClass('etalage_thumb_active').show().stop().css({ opacity:0 }).animate({ opacity:1 }, o.speed);
					...
					if(typeof etalage_change_callback === 'function'){
						var image_number = $next_active.data('id');
						if(images >= o.small_thumbs){
							image_number--;
						}
...
// ACTIONS
				...
				var max_zoomx = -(o.source_image_width-o.zoom_area_width),
					max_zoomy = -(o.source_image_height-o.zoom_area_height);
				...
				// Large thumbnail click callback
					...
					$magnifier.click(function(){
						...
						if(typeof image_anchor!=='undefined' && image_anchor){
